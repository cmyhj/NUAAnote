...
| **[[ROS Topic（话题通信）|Topic]]** | 异步、发布-订阅模型；一对多/多对多通信；有缓冲区；实时性弱 |
| **[[ROS Service（服务通信）|Service]]** | 同步、客户端-服务器模型；一对多（服务端唯一）；无缓冲区；实时性强 |
| **[[ROS Parameter Server（参数服务器）|Parameter Server]]** | 由 ROS Master 管理的全局变量存储；仅用 RPC 通信（3 步），无 TCP |
